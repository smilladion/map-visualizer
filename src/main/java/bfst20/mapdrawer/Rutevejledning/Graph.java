    ...
    public Graph(int vertices, List<OSMWay> highways) {
        ...
        for (OSMWay way : highways) {
            ...
            for (int i = 0; i < way.getNodes().size() - 1; i++) {
                ...
                addEdge(from, to, weight, bike, walk, car, way);
                addEdge(to, from, weight, bike, walk, car, way);
            }

        }
    }

    public void addEdge(int from, int to, double weight, boolean bike, boolean walk, boolean car, OSMWay way) {
        DirectedEdge edge = new DirectedEdge(from, to, weight, bike, walk, car, way);
